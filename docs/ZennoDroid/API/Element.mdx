...
bool clickable = de.GetBooleanAttribute("clickable"); // Можно ли нажать на элемент
bool enabled = de.GetBooleanAttribute("enabled"); // Активен ли элемент
```
...
var data = Convert.FromBase64String(de.GetScreenshot()); // Скриншот области элемента
...
de.SendKeys("ZennoDroid"); // Отправить текст элементу
```
