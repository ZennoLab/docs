...
- **`void WaitForUserAction(int timeout)`**  
    Останавливает проект и передаёт устройство человеку: в верхней части окна устройства появляется полоса с кнопкой «Продолжить», и на устройстве можно вручную сделать то, что шаблон делать не должен. То же, что экшен [**Ожидание действий пользователя**](../project-editor/logic/Wait_for_User_Action).
    
    Проект стоит на этой строке, пока человек не нажмёт «Продолжить», пока не выйдет таймаут или пока шаблон не остановят.
    
    **Параметры:**
    - `timeout` — сколько секунд отводится на ручные действия. Отрицательное значение — ждать, пока человек не нажмёт «Продолжить».

- **`void WaitForUserAction(int timeout, string comment)`**  
    То же самое, с сообщением для человека.
    
    **Параметры:**
    - `timeout` — сколько секунд отводится на ручные действия. Отрицательное значение — ждать, пока человек не нажмёт «Продолжить»;
    - `comment` — текст, который увидит человек в окне устройства. Сюда пишут, что именно нужно сделать.

#### Пример.  
```csharp
var action = instance.DroidInstance.Action;

action.WaitForUserAction(120, "Введите логин и пароль вручную"); // Две минуты на ручные действия
```  
```csharp
var action = instance.DroidInstance.Action;

action.WaitForUserAction(-1); // Ждать, пока человек не нажмёт «Продолжить»
```
