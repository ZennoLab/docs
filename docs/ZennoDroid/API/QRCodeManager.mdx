...
qr.CloseQRCodeScanWindow(); // Закрыть окно сканирования QR-кода
```
...
qr.CloseViewfinderWindow(); // Закрыть окно видоискателя
```
...
var area = new Rectangle(100, 200, 400, 400); // Куда поместить изображение
var background = new Rectangle(0, 0, 720, 1280); // Область фона

qr.DrawImage(project.Path + "qrcode.png", area, background); // Из файла
qr.DrawImage(File.ReadAllBytes(project.Path + "qrcode.png"), area, background); // Из массива байтов
```
...
qr.SetViewfinderPosition(new Rectangle(100, 200, 400, 400)); // Разместить окно видоискателя
```
