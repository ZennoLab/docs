...
- **`string GetRawSource()`**  
    Возвращает исходный XML-код текущего экрана в том виде, в котором его отдаёт драйвер Appium, без постобработки. В отличие от [`Hierarchy.GetLayout()`](./Hierarchy#getlayout), имена тегов сохраняют полные префиксы Java-пакетов (например, `android.widget.TextView`).
    
    **Возвращает:**  
    XML-строку с иерархией элементов интерфейса текущего экрана.

#### Пример.  
```csharp
var driver = instance.DroidInstance.AppiumDriver;

var xml = driver.GetRawSource(); // Получить необработанный XML текущего экрана
```
...
var data = Convert.FromBase64String(driver.GetScreenshot());
