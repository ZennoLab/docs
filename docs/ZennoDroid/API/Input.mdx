...
input.FingerDown(100, 500); // Опустить палец на экран
input.FingerMove(150, 420); // Каждый вызов - одна точка траектории
input.FingerMove(220, 300);
input.FingerMove(300, 200);
input.FingerUp(300, 200); // Убрать палец
```

### FingerMove

- **`void FingerMove(int x, int y)`**  
    Перемещает активный палец в новую точку. Траектория между предыдущей и новой точкой не воспроизводится: палец оказывается в указанной точке сразу, поэтому для непрерывного движения метод вызывают для каждой точки пути. Готовую траекторию строят [**Swipe**](#swipe), [**SwipeCurved**](#swipecurved) и [**FreeMove**](#freemove).
...
// Один вызов перемещает палец сразу в указанную точку, поэтому линию
// задаём сами - точка за точкой
input.FingerDown(100, 500);
for (int x = 110; x <= 300; x += 10)
	input.FingerMove(x, 500 - (x - 100));
input.FingerUp(300, 300);

// Мультитач: у каждого пальца свой fingerId
...
input.FingerDown(100, 500); // Опустить палец
input.FingerUp(100, 500); // Убрать в той же точке - получится обычное нажатие
```
...
// Траектория - массив жестов, в каждом жесте точки вида [время в мс, X, Y]
var json = "[[[0,100,500],[100,150,420],[200,220,300],[300,300,200]]]";

input.FreeMove(json, 1); // Скорость 1 - как в траектории
input.FreeMove(json, 2); // Скорость 2 - вдвое быстрее
```
...
input.LongTap(point.X, point.Y); // Долгое нажатие по координатам
```
...
Point start = new Point() { X = 100, Y = 500 }; // Точка старта
Point end = new Point() { X = 300, Y = 200 }; // Точка конца

input.LongTapAndSwipeCurved(start.X, start.Y, end.X, end.Y); // Долгий тап и свайп по кривой
input.LongTapAndSwipeCurved(start.X, start.Y, end.X, end.Y, 500); // С длительностью в мс
input.LongTapAndSwipeCurved(start.X, start.Y, 0.5, end.X, end.Y); // С коэффициентом изгиба
input.LongTapAndSwipeCurved(start.X, start.Y, 200, 250, end.X, end.Y); // Через контрольную точку
```
...
var model = input.RootShell("getprop ro.product.model"); // Выполнить команду с правами root
var packages = input.RootShell("pm list packages", 30000); // С таймаутом в мс
...
- **`void SetNativeEventMode(bool useNativeEvent)`**  
    Включает или выключает режим нативных событий. Когда режим включён (и процесс действительно выполняется с правами root), касания вводятся через уровень ввода ядра (raw), благодаря чему они становятся видимыми для `getevent`.
    
    **Параметры:**
    - `useNativeEvent` — `true`, чтобы вводить касания через уровень ввода ядра; `false` — использовать способ по умолчанию.

#### Пример.  
```csharp
var input = instance.DroidInstance.Input;

input.SetNativeEventMode(true); // Включить ввод касаний через уровень ввода ядра
input.Tap(100, 500); // Касание будет видно через getevent
```
