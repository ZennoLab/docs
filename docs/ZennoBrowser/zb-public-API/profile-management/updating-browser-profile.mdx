...
| imagesVideosDownload | string |  | (empty) | Настройка загрузки изображений и видео. Допустимые значения: `on` — включить, `off` — отключить. Пустая строка сохраняет текущее значение. |
| windowSizeFromFingerprint | string |  | (empty) | Открывать окно браузера точно в разрешении экрана из отпечатка профиля. Допустимые значения: `on` — включить, `off` — отключить. Пустая строка сохраняет текущее значение. |
