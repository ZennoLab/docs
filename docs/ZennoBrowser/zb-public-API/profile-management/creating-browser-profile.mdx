...
Этот метод создаёт новый профиль браузера в указанном рабочем пространстве и папке. Можно настраивать разные параметры, такие как размер экрана, эмуляция железа, язык, часовой пояс, прокси и т.д. Все параметры необязательные, если отдельно не указано иное.
...
| cookies | string |  | (пусто) | Настройки конфигурации cookies |
...
```bash
curl 'http://localhost:8160/v1/profiles/create?name=ApiProfile&workspaceId=-1&folderId=&screen=&cpu=&memory=&language=&geoLocation=&timeZone=&webGl=&webGpu=&webRtc=&domRect=&audio=&fonts=&battery=&plugins=&proxyServerId=&speechVoices=&cookies=&notes=&tags=' \
  --request POST \
...
var request = new HttpRequestMessage
{
    ...
    RequestUri = new Uri("http://localhost:8160/v1/profiles/create?name=ApiProfile&workspaceId=-1&folderId=&screen=&cpu=&memory=&language=&geoLocation=&timeZone=&webGl=&webGpu=&webRtc=&domRect=&audio=&fonts=&battery=&plugins=&proxyServerId=&speechVoices=&cookies=&notes=&tags="),
    Headers =
    {
