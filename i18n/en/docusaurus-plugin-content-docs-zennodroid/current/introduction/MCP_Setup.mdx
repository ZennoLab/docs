...
Everything above is only needed for **external** assistants. The AI chat built into ProjectMaker ZennoDroid works with the same MCP servers out of the box — all it needs from you is a model API key. Details are in the [AI assistant in ZennoDroid](./ai-assistant) article.
:::
