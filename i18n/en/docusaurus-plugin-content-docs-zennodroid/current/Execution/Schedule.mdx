---
sidebar_position: 4
sidebar_label: «Schedule» Tab
title: «Schedule» Tab | ZennoDroid Documentation | ZennoLab
description: «Schedule» Tab - section of the official ZennoDroid documentation by ZennoLab. Detailed guides, setup instructions, and usage examples
---  
...
- **Display log**. Show the history of attempt additions by the scheduler.
...
- **By date**. The task will run at a specified date and time.
...
You can enter a specific number or a range (with the **Specify range** button). In the second case, the system will randomly pick a number within that range.
...
- **One after another**. The task restarts as soon as the last run finishes.
- **One after another with a pause**. Now there's a pause between repeats. You can set a specific number of minutes, or **Set range** and let the system pick a random value.  
...
- **Distribute over an interval**. Repeats are automatically distributed within the intervals you set in **When to Repeat**.
...
- **Endless**. The project keeps running indefinitely, until you stop it manually.
_______________________________________________
## Example Setups  
### One time  
| Run the project *one time* on a certain day at 03:25 AM. | 
| -------- |
| ![Every day](./assets/Schedule/Schedule_pic14.png) |
### Every day
| Run the task *every day* from 09:00 AM to 03:00 PM, only once per day, with a 10-minute pause between runs.    | 
| -------- | 
| ![Every day](./assets/Schedule/Schedule_pic15.png)  |  

### Multiple Attempts
| Run the template every day from 8:00 AM to 12:00 PM and 01:00 PM to 07:00 PM, repeating it every 20-40 minutes, *doing between one and seven attempts*.    | 
| -------- | 
| ![Multiple attempts](./assets/Schedule/Schedule_pic16.png)  | 

### Random Distribution
| Run the project every Tuesday, Wednesday, and Thursday from 04:00 PM to 11:00 PM, *randomly distributing* 50 repeats across the time slot.    | 
| -------- | 
| ![Random distribution](./assets/Schedule/Schedule_pic17.png)  |  
_______________________________________________
## Example Video  
<VideoSample source={require("@site/static/video/iTe1y6fqwL.mp4").default}/>
_______________________________________________
## Schedule Debugger
### General Info  
