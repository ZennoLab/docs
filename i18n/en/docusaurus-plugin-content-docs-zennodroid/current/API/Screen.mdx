...
- **`Rectangle FindImage(string imageHash, Rectangle[] rectangles, int confidenceInterval, int searchTime)`**  
    ...
    **Parameters:**
    ...
    - `searchTime` — maximum search time in seconds.
...
var hash = project.Variables["imageHash"].Value; // Hash of the reference image
var areas = new[] { new Rectangle(0, 0, 720, 1280) }; // Where to search - the whole screen here

var found = screen.FindImage(hash, areas, 90); // Match accuracy of 90
if (found.IsEmpty)
	throw new Exception("Image not found");

var waited = screen.FindImage(hash, areas, 90, 10); // Wait up to 10 seconds for it to appear
```
...
var hash = project.Variables["imageHash"].Value; // Hash of the reference image
var areas = new[] { new Rectangle(0, 0, 720, 1280) }; // Search area

var found = screen.FindImageBlackWhite(hash, areas, 90, 128); // Binarization threshold of 128
if (found.IsEmpty)
	throw new Exception("Image not found");
```
...
var found = screen.FindImageGrayscale(hash, areas, 90); // Compared in grayscale
if (found.IsEmpty)
	throw new Exception("Image not found");
```
...
var point = screen.FindPixel("#FF0000", areas, 10); // A red pixel, deviation of 10
if (point.IsEmpty)
	throw new Exception("Pixel not found");

var waited = screen.FindPixel(Color.Red, areas, 10, 10); // The same, but waiting up to 10 seconds
```
...
var areas = new[] { new Rectangle(0, 0, 720, 1280) }; // Search area
var colors = new[] { "#FF0000", "#00FF00" }; // Any one of the colours will do

var point = screen.FindPixels(colors, areas, 10);
if (point.IsEmpty)
	throw new Exception("None of the colours was found");
```
...
- **`Size GetScreenSize()`**  
    Returns the real screen resolution of the device in pixels. This is the physical resolution the device reports, not the size of the captured image, so it is the coordinate space used by touch, swipe and pixel operations.
    
    **Returns:**  
    `Size` — screen width and height in pixels.

#### Example
```csharp
var size = instance.DroidInstance.Screen.GetScreenSize();
project.SendInfoToLog($"Screen resolution: {size.Width}x{size.Height}");
```

:::note
The value is cached per device. After a resolution change (e.g. with `wm size WidthxHeight`), call `ResetScreen()` so it is read again.
:::
...
instance.DroidInstance.Action.Start("off"); // Start the device without continuous capture
screen.RefreshScreen(); // Update the image manually
```
...
instance.DroidInstance.Input.RootShell("wm size 720x1280"); // Change the screen resolution
screen.ResetScreen(); // Restart the capture for the new resolution
```
...
var base64 = screen.ScreenshotAsBase64String(); // The whole screen
var area = screen.ScreenshotAsBase64String(new Rectangle(0, 0, 300, 300)); // One area only
...
screen.SetCaptureScreenQuality(10); // Highest quality - more accurate search, but slower
screen.SetCaptureScreenQuality(0); // Lowest quality - faster
```
