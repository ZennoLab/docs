...
```  
_______________________________________________
### Start the selected device using profile data
```csharp
void Start(bool applyProfile)  
```  
Device parameters (IMEI, Android ID, etc.) will be taken from the ZennoDroid profile (the **Current profile** tab).  
...
var action = instance.DroidInstance.Action;
action.SelectByTitle("Name"); // Select the device.
action.Start(true, "mjpeg"); // Start with captureScreenMethod.
```
_______________________________________________
