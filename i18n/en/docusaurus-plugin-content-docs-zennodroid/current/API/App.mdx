...
```
_______________________________________________
### Injecting custom javascript before page load.
```csharp
...
*Parameters:*   
```csharp
string packageName // Application name for the method to work.
string script // javascript to be executed before the page loads
``` 
...
instance.DroidInstance.App.Close(app); // Close the application

var lspHelper = new LSPosedHelper(instance.DroidInstance);
lspHelper.SetPackages(app); // Set the application in the ZennoDroid module spoofing list
lspHelper.EnableModule(); // Enable the ZennoDroid module

instance.DroidInstance.Settings.SetLSPosedSettings("{ \\"UseInjectScript\\":true }"); // Enable the ability to execute javascript before page load
instance.DroidInstance.App.InjectScript(app, script); // Set the script to be executed before loading

instance.DroidInstance.App.Open(app); // Open the application
```
The project variable script contains the code to be executed.
```javascript
(() => {
