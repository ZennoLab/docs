...
input.FingerDown(100, 500); // Put a finger down on the screen
input.FingerMove(150, 420); // Every call is one point of the path
input.FingerMove(220, 300);
input.FingerMove(300, 200);
input.FingerUp(300, 200); // Lift the finger
```

### FingerMove

- **`void FingerMove(int x, int y)`**  
    Moves the active finger to a new point. The path between the previous point and the new one is not replayed: the finger lands on the given point at once, so a continuous movement means calling the method for every point of the path. A ready-made trajectory is built by [**Swipe**](#swipe), [**SwipeCurved**](#swipecurved) and [**FreeMove**](#freemove).
...
// A single call puts the finger straight at the point, so the line
// has to be laid out point by point
input.FingerDown(100, 500);
for (int x = 110; x <= 300; x += 10)
	input.FingerMove(x, 500 - (x - 100));
input.FingerUp(300, 300);

// Multi-touch: every finger has its own fingerId
input.FingerDown(100, 500, 0);
input.FingerDown(400, 500, 1);
input.FingerMove(200, 500, 0);
input.FingerMove(300, 500, 1);
input.FingerUp(200, 500, 0);
input.FingerUp(300, 500, 1);
```
...
input.FingerDown(100, 500); // Put a finger down
input.FingerUp(100, 500); // Lift it at the same point, which makes a plain tap
```
...
// The trajectory is an array of gestures, each a list of [time in ms, X, Y] points
var json = "[[[0,100,500],[100,150,420],[200,220,300],[300,300,200]]]";

input.FreeMove(json, 1); // Speed 1 plays the trajectory as recorded
input.FreeMove(json, 2); // Speed 2 plays it twice as fast
```
...
input.LongTap(point.X, point.Y); // Long tap at the coordinates
```
...
Point start = new Point() { X = 100, Y = 500 }; // Start point
Point end = new Point() { X = 300, Y = 200 }; // End point

input.LongTapAndSwipeCurved(start.X, start.Y, end.X, end.Y); // Long tap and a curved swipe
input.LongTapAndSwipeCurved(start.X, start.Y, end.X, end.Y, 500); // With a duration in ms
input.LongTapAndSwipeCurved(start.X, start.Y, 0.5, end.X, end.Y); // With a bending factor
input.LongTapAndSwipeCurved(start.X, start.Y, 200, 250, end.X, end.Y); // Through a control point
```
...
var model = input.RootShell("getprop ro.product.model"); // Run a command with root rights
var packages = input.RootShell("pm list packages", 30000); // With a timeout in ms

project.SendInfoToLog(model);
```
...
- **`void SetNativeEventMode(bool useNativeEvent)`**  
    Enables or disables native-event mode. When enabled (and the process actually runs as root), finger touches are injected through the kernel input layer (raw), which makes them visible to `getevent`.
    
    **Parameters:**
    - `useNativeEvent` — `true` to inject touches via the kernel input layer; `false` to use the default method.

#### Example  
```csharp
var input = instance.DroidInstance.Input;

input.SetNativeEventMode(true); // Inject touches via the kernel input layer
input.Tap(100, 500); // The touch will be visible to getevent
```
