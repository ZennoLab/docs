...
qr.CloseQRCodeScanWindow(); // Close the QR code scan window
```
...
qr.CloseViewfinderWindow(); // Close the viewfinder window
```
...
var area = new Rectangle(100, 200, 400, 400); // Where the image goes
var background = new Rectangle(0, 0, 720, 1280); // Background area

qr.DrawImage(project.Path + "qrcode.png", area, background); // From a file
qr.DrawImage(File.ReadAllBytes(project.Path + "qrcode.png"), area, background); // From a byte array
```
...
qr.SetViewfinderPosition(new Rectangle(100, 200, 400, 400)); // Place the viewfinder window
```
