...
var de = driver.FindElementByXPath("//*[@text=\"Display\"]"); // Find the element by xPath
if (de == null)
    throw new Exception("Element not found");

bool clickable = de.GetBooleanAttribute("clickable"); // Whether the element can be clicked
bool enabled = de.GetBooleanAttribute("enabled"); // Whether the element is enabled
```
...
var data = Convert.FromBase64String(de.GetScreenshot()); // Screenshot of the element's area
using (var ms = new MemoryStream(data))
using (var bmp = new Bitmap(ms))
    bmp.Save(project.Path + "element.png");
```
...
de.SendKeys("ZennoDroid"); // Send text to the element
```
