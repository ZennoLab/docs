...
- *Save the result to a variable.* Receives a JSON array: name, type, password and metadata for every account. The metadata from this result is what the **Add account** action expects.  
...
- *Account password.* Goes in encrypted, exactly as the **Get accounts** action returns it - a plain-text password will not do. An empty field means no password is set.  
- *Account metadata.* The extra data of the account exactly as the **Get accounts** action returned it: that is what carries the account, authorisation and all, to another device. Arbitrary text or your own JSON will not do.  
:::warning **Root is required.**
:::
_______________________________________________ 
### Remove account  
