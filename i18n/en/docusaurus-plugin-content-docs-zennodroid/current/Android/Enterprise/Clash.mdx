---
sidebar_position: 15
sidebar_label: Installing Clash (Box for Root)
title: Installing Clash (Box for Root) | ZennoDroid Documentation | ZennoLab
description: Installing Clash - section of the official ZennoDroid documentation by ZennoLab. Detailed guides, setup instructions, and usage examples
---
import DisclaimerNotice from '@site/src/components/DisclaimerNotice';


# Installing Clash (Box for Root)
<DisclaimerNotice />
...
:::warning **Clash needs BusyBox version 1.36.1 or newer.**
On emulators ZennoDroid puts its own copy of busybox in place, while on a real device BusyBox has to be installed by hand — see [**Installing BusyBox**](../../Enterprise/BusyBox). Otherwise installing Clash ends with the *Please Update BusyBox to version 1.36.1+* error.
:::
_______________________________________________
## Automatic Installation
The **Box for Root** module is installed automatically when configuring a proxy in Clash mode for the first time.

### Automatic Updating
If Clash is already installed on the device, it will not update automatically.

To update an installed version, you need to perform the following steps:
1. **Remove the installed Clash**. To do this, execute the Adb Shell action *with superuser (root) privileges enabled*: `rm -rf /data/adb/box`
2. After that, set up the proxy in Clash mode. **The new version will be installed automatically**.
_______________________________________________
## Manual Installation  
