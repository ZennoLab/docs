---
sidebar_position: 2
sidebar_label: List Processing
title: List Processing | ZennoDroid Documentation | ZennoLab
description: List Processing - section of the official ZennoDroid documentation by ZennoLab. Detailed guides, setup instructions, and usage examples
---
...
From the context menu: ***Add action → Lists → List processing***.
...
- **Range.** How you want to filter the sublist. You can also pick:  
    ...
    - *Elements matching regex.*
    Set the search criteria using Regex (regular expressions).
    - *Elements not containing text.*
    This will pick all lines that don't have the given text. You can use variables.
    - *Elements not matching regex.*
    Set the search criteria using Regex (regular expressions).
- **Field for criteria.** Enter the values you want based on the previous point.
- **Delete rows after taking.** Lines that match the criteria will be deleted.
- **Exit by failure, if list is empty.** If the list is empty, the project will switch to the red branch.
_______________________________________________
...
- **Separator.**
    ...
    - *Specified in the list.* Uses the separator from the list settings.
- **Put to variable.** Variable for storing the result after processing.
_______________________________________________
...
- **Filter options:**
    ...
    - *Random;*
    - *Matching regex;*
- **Delete line after taking.** Once you’ve taken a line from the list, it’ll be deleted after.
- **Put to variable.** Here you set your custom variable where the copied line will go.
_______________________________________________
### Bind to the file
Links the list to a file while the project runs.

- **File path.** Pick a specific file or set a variable with the path.
- **Create a file if it does not exist.** If there’s no file at the given path, one will be created automatically.
_______________________________________________
### Sort
Sorts list items in ascending or descending order.

- **Sort content as numbers if it is possible.** Use numeric sorting when you can.
...
- **File path.** Pick a file or set a variable with the file path.
