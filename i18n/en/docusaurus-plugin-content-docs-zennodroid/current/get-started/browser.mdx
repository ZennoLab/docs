---
sidebar_position: 1
sidebar_label: Getting Started with the Browser
title: Getting Started with the Browser | ZennoDroid Documentation | ZennoLab
description: Getting Started with the Browser - section of the official ZennoDroid documentation by ZennoLab. Detailed guides, setup instructions, and usage examples
---
...
Right-click the block → Add action → [Android → Keyboard Emulation](../Android/pro-lite/Keyboard). In the action properties, enter: `AndroidKeys.ENTER`.
...
### 5. Open and Minimize
Right-click the block with the event that opens the link → **Add action → [Logic → Pause](../project-editor/Logic/Pause) → set 10 seconds**. After that, add a [**Keyboard Emulation**](../Android/pro-lite/Keyboard) action with the special button, which will minimize the app. The action's text will automatically be `AndroidKeys.HOME`.
...
1. Right-click the blocks area → **Add action → [Android → App Actions](../Android/pro-lite/App#saving-of-applications-data).**
...
2. Repeat the last step, but pick [**Restore app data**](../Android/pro-lite/App#restoring-of-applications-data) as the action. Specify the path to your backup archive.
...
## Useful Links
- [**App Actions**](../Android/pro-lite/App)
- [**Pause**](../project-editor/Logic/Pause)
