---
sidebar_position: 1
sidebar_label: System Requirements
title: System Requirements | ZennoDroid Documentation | ZennoLab
description: System Requirements - section of the official ZennoDroid documentation by ZennoLab. Detailed guides, setup instructions, and usage examples
---
import DisclaimerNotice from '@site/src/components/DisclaimerNotice';


# System Requirements
<DisclaimerNotice />

## For Lite and Professional versions.  
### At a minimum, you'll need:
- **Processor:** Intel or AMD with at least 2 cores, each at 2 GHz or faster.  
- **RAM:** at least 4 GB.  
- **Storage space:** at least 6 GB free.  
- **Video memory:** at least 1024 MB.  
- **Direct X 11+ or OpenGL 2.0 required.**  
- **Operating system:** Windows 10 x64 or later  
Windows Server 2019–2025 (Server Core not supported).  
- **.Net Framework 2.0, 3.5, and [4.8+](https://dotnet.microsoft.com/download/dotnet-framework/net48) are required.**  
- **Visual C++ redistributables for 2008, 2010, 2013, 2015, 2017, 2019 (x86 or x64) are also needed.**  
- **Administrator rights.** You'll need them to install and run the program.  
- **Hardware virtualization.**  
- **Internet connection.** Mandatory. The program won't work without it.  
:::warning **[Hyper-V must be disabled!](../pro-lite/Hyper-V)**
:::
_______________________________________________  
## For the Enterprise version.  
### At a minimum, you'll need:  
- **Processor:** Intel or AMD with at least 2 cores, each at 2 GHz or faster.  
- **RAM:** at least 4 GB.  
- **Storage space:** at least 6 GB free.  
- **Video memory:** discrete graphics card required.  
- **Operating system:** Windows 10 x64 or later  
Windows Server 2019–2025 (Server Core not supported).  
- **.Net Framework [4.8+](https://dotnet.microsoft.com/download/dotnet-framework/net48) required.**  
- **Visual C++ redistributables for 2013, 2015, 2017, 2019, and 2022 also needed.**  
- **Administrator rights.** You'll need them to install and run the program.  
- **Internet connection.** Mandatory. The program won't work without it.  
_______________________________________________   
## How do you check your system with Memu?  
Launch the emulator, go to the menu, and select **System Info**. 

![Memu menu](./assets/SysReq/SysReq_pic1.png)   

It will look like this:  

![Memu menu](./assets/SysReq/SysReq_pic2.png)  
_______________________________________________
## Useful links  
- [**Installing Memu**](../pro-lite/Memu_Install)  
- [**Installing ZennoDroid**](./ZD_Install)  
- [**ZennoDroid Demo Version**](./ZD_Demo)