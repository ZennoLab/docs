---
sidebar_position: 9
sidebar_label: Translators
title: Translators | ZennoDroid Documentation | ZennoLab
description: Translators - section of the official ZennoDroid documentation by ZennoLab. Detailed guides, setup instructions, and usage examples
---
import DisclaimerNotice from '@site/src/components/DisclaimerNotice';

# Translators
<DisclaimerNotice />

## Description.
In this settings section, you enter the information needed to use automatic translation services in your projects.
**They’re needed for:**
- *translating articles;*
- *translating message text into the recipient’s language;*
- *translating pre-made text for autobot replies.*

![Description](./assets/Translators/Translators_pic1.png)
_______________________________________________
## Baidu Translate.
![Baidu Translate](./assets/Translators/Translators_pic2.png)

:::info **The platform’s native language is Chinese.**
For easier setup, we recommend using a browser with a built-in translator.
:::

### How to get your secret key.
#### 1. Go to the [service website](https://fanyi.baidu.com/).
#### 2. Hover your cursor over the indicated character:
![Baidu Translate](./assets/Translators/Translators_pic3.png)
#### 3. A pop-up window for login will appear:
![Baidu Translate](./assets/Translators/Translators_pic4.png)
#### 4. Log in to the service.
![Baidu Translate](./assets/Translators/Translators_pic5.png)

**1**. Enter your username and password here.  
**2**. Button to log in.  
**3**. Password recovery.  
**4**. Go back to app login.  
**5**. Create an account.  

#### 5. Create a new account.
In the login window, click **Register**:

![Baidu Translate](./assets/Translators/Translators_pic6.png)

A page will open for entering your data to create an account:

![Baidu Translate](./assets/Translators/Translators_pic7.png)

**1**. Username.  
**2**. Mobile number without country code.  
:::info **Only Chinese numbers are accepted.**  
:::
**3**. Password that meets the service requirements.  
**4**. Button to send authentication SMS.  
**5**. Field for SMS code.  
**6**. Agree to the terms of service.  
**7**. Register.  

After registering, you’ll be able to copy the key from your account and paste it in the **ZennoDroid Settings**.
_______________________________________________
## Deepl Translate.
![Deepl Translate](./assets/Translators/Translators_pic8.png)

### Getting your secret key.
#### 1. Go to the [service site](https://www.deepl.com/en/translator) and click on the **Pricing** tab.
![Deepl Translate](./assets/Translators/Translators_pic9.png)
#### 2. Select **DeepL API** and then choose one of the two plans: DeepL API Free *(free)* or DeepL API Pro *(4.99€)*.
![Deepl Translate](./assets/Translators/Translators_pic11.png)
#### 3. Now enter your **email** and create a **password**.
![Deepl Translate](./assets/Translators/Translators_pic13.png)
#### 4. Next, fill in your personal details *(country, address, first and last name)*.
![Deepl Translate](./assets/Translators/Translators_pic14.png)
#### 5. Then enter your card details.
![Deepl Translate](./assets/Translators/Translators_pic15.png)

Even if you choose the free plan, your card still has to be valid in order for the registration to go through.
#### 6. Once you’ve finished signing up, go to your Personal Account → API Keys.
![Deepl Translate](./assets/Translators/Translators_pic16.png)

Copy your API key and paste it into the ZennoDroid settings.
_______________________________________________
## Google Translate via API
![Google Translate](./assets/Translators/Translators_pic17.png)

:::tip **We recommend reviewing the rates and fees before using.**
:::

### Getting your secret key.
#### 1. First, sign in to your Google account.
![Google Translate](./assets/Translators/Translators_pic18.png)
#### 2. Then create your first project on the [**Google Cloud**](https://console.cloud.google.com/projectcreate) platform.
![Google Translate](./assets/Translators/Translators_pic19.png)
#### 3. Now, select your project and add [**Cloud Translation API**](https://console.cloud.google.com/apis/library/translate.googleapis.com?inv=1&invt=AbeTEA&project=sonic-verbena-438109-p0).
![Google Translate](./assets/Translators/Translators_pic20.png)

:::info **You’ll need to set up a billing account first.**
:::

#### 4. Get your API key and add it to ZennoDroid’s settings.
_______________________________________________
## Google Translate Web Interface.
No setup needed for this method, just use the [**online translator**](https://translate.google.com/) in your browser.

:::tip **Works for free.**
:::
_______________________________________________
## Microsoft Translation.

![Microsoft Translation](./assets/Translators/Translators_pic22.png)

:::tip **We recommend reviewing the rates and fees before using.**
:::

### Getting your API key.
#### 1. Go to [**Microsoft Azure**](https://azure.microsoft.com/en-gb/pricing/purchase-options/azure-account/), log in, or create a new account.
![Microsoft Translation](./assets/Translators/Translators_pic23.png)

:::tip **A 30-day trial is available.**
:::

#### 2. After you get the trial period, go to the [Azure Services](https://portal.azure.com/) page and **create your resource**.
![Microsoft Translation](./assets/Translators/Translators_pic24.png)

#### 3. Use the search bar to find Translator, open it, and click Create.
![Microsoft Translation](./assets/Translators/Translators_pic26.png)

#### 4. Get your API key and paste it into ZennoDroid.
_______________________________________________
## Yandex Translate.
![Yandex Translate](./assets/Translators/Translators_pic27.png)

### Getting your secret key.
#### 1. Sign up at [**Yandex Cloud**](https://yandex.cloud/en/services/translate).
![Yandex Translate](./assets/Translators/Translators_pic28.png)

:::tip **A welcome grant is awarded to test out the platform.**
:::

#### 2. After logging in, go to your [**personal dashboard**](https://console.yandex.cloud/folders/) and create a billing account.
![Yandex Translate](./assets/Translators/Translators_pic29.png)

#### 3. Now you need to create a [**service account**](https://yandex.cloud/en/docs/iam/concepts/users/service-accounts).
![Yandex Translate](./assets/Translators/Translators_pic30.png)

#### 4. Next, on your [**dashboard**](https://console.yandex.cloud/folders/), go to the Access Bindings section.
![Yandex Translate](./assets/Translators/Translators_pic31.png)

#### 5. Go to the service account and generate an API key.
![Yandex Translate](./assets/Translators/Translators_pic32.png)

:::warning **API key value can be viewed only once — immediately after creation.**
:::
_______________________________________________
## Example of use.
While working on a project, you can translate any content you’re using. For example, you can translate a phrase from Russian to English to send in a message.

![Example of use](./assets/Translators/Translators_pic33.png)

1. Get the text string from a [**List**](../project-editor/lists-and-sheets/Lists).
2. Add the [**Text Processing**](../Data/Text) action.
3. Set up the translation function, selecting the desired service and options.
4. Send the translated message to the recipient.