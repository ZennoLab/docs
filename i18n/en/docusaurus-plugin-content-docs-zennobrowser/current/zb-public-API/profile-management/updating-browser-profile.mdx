...
| imagesVideosDownload | string |  | (empty) | Images and videos download configuration. Allowed values: `on` to enable or `off` to disable. Use an empty string to keep the current value. |
| windowSizeFromFingerprint | string |  | (empty) | Open the browser window exactly in the profile fingerprint screen resolution. Allowed values: `on` to enable or `off` to disable. Use an empty string to keep the current value. |
...
**Cube:**  
```
http://localhost:8160/v1/profiles/{profileId}?workspaceId=-1&folderId=a2f76986-8064-4c2f-b109-64a84b4ce137&name=NewName&notes=Note123&tags=tag1 tag2&imagesVideosDownload=on&windowSizeFromFingerprint=on
```
