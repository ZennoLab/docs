...
title: Basic Concepts | ZennoProxy Documentation | ZennoLab
description: Key concepts for working with ZennoProxy — section of the official ZennoProxy  documentation by ZennoLab. Detailed guides, setup instructions, and usage examples.
--- 
...
Static and rotating sessions determine how an IP address is used when interacting with websites. A static session means using and keeping the same IP address for as long as possible (up to the maximum allowed period before a forced change by the provider or network), while a rotating session involves changing the IP address with each new request or at specific intervals.  
